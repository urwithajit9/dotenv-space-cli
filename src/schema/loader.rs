...
use super::models::{Schema, ServiceConfig, FrameworkConfig, StackBlueprint};
...
pub fn schema() -> Result<&'static Schema> {
    ...
    SCHEMA.get().ok_or_else(|| anyhow::anyhow!("Schema failed to initialize"))
}
...
pub fn schema_fallback() -> Result<&'static Schema> {
    ...
    FALLBACK.get_or_init(|| {
        serde_json::from_str(SCHEMA_JSON)
            .expect("schema.json must be valid JSON")
    });
    FALLBACK.get().ok_or_else(|| anyhow::anyhow!("Schema not initialized"))
}
...
pub fn find_service(service_id: &str) -> Option<(&str, &ServiceConfig)> {
    ...
    schema.services.databases.get(service_id)
...
pub fn get_services_grouped() -> Vec<(&'static str, Vec<(&'static str, String)>)> {
    ...
    vec![
        ("Databases", extract_service_names(&schema.services.databases)),
        ("Messaging", extract_service_names(&schema.services.messaging_queues)),
        ("Auth", extract_service_names(&schema.services.auth_providers)),
        ("Storage", extract_service_names(&schema.services.storage)),
        ("Monitoring", extract_service_names(&schema.services.monitoring_logging)),
        ("Payments", extract_service_names(&schema.services.payments)),
        ("AI/ML", extract_service_names(&schema.services.ai_ml)),
        ("Email/SMS", extract_service_names(&schema.services.email_sms)),
    ]
    .into_iter()
